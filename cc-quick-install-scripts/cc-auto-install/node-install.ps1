...
<#
    这个脚本会检查当前系统中安装的 Node.js 版本，如果版本过低（低于 v18），它将自动下载并安装 Node.js v18 或 v22（用户选择）。安装完成后，它还会设置一些环境变量以优化 Claude Code 的运行环境。

    注意：如果你已经安装了 Node.js，并且版本满足要求，你可以选择跳过安装步骤。
...
function Install-WithNVM {
    ...
    if (-not $nvmInstalled) {
        # $nvmInstallerUrl = "https://github.com/coreybutler/nvm-windows/releases/download/failure-test"
...
function Main {
    ...
    # Simplify: No need to select the method, try the NVM first, if it fail, then use the MSI.
    ...
    # }
    if (-not $success) {
        Write-ColorOutput Yellow "Installing with the NVM was failed. Now, trying use the MSI installer."
        $success = Install-NodeJS -Version $fullVersion -MajorVersion $version
    }
    ...
    if ($success) {
        ...
        if (Verify-Installation) {
            ...
            Write-ColorOutput Green "node-v$fullVersion has installed."

            # Optional: Change the npm mirror source (Or you can run the npm-mirror-switcher.ps1 to choose)
            Write-ColorOutput Yellow "Setting the npm mirror source ..."
